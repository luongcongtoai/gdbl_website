...
class filterClass extends acymailingClass{
	...
	function addJSFilterFunctions(){
		$js = " var numFilters = 0;
				...
				function displayCondFilter(fct, element, extra){";
					$ctrl = 'filter';
					if(!$app->isAdmin()) $ctrl = 'frontfilter';
					$js .= "
					try{
						new Ajax('index.php?option=com_acymailing&tmpl=component&ctrl=".$ctrl."&task=displayCondFilter&fct='+fct+'&'+extra,{
							method: 'get',
							update: document.getElementById(element)
						}).request();
					}catch(err){
						new Request({
							url:'index.php?option=com_acymailing&tmpl=component&ctrl=".$ctrl."&task=displayCondFilter&fct='+fct+'&'+extra,
							method: 'get',
							onSuccess: function(responseText, responseXML) {
